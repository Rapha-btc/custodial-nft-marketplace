;; SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.lambda-nft-marketplace
...
(define-public (initialize (nft-contract principal) (name (string-ascii 128)))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (asserts! (is-none (var-get allowed-nft)) ERR-ALREADY-INITIALIZED)
    (var-set allowed-nft (some nft-contract))
    (try! (as-contract? ()
      (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-nfts-core register-marketplace
        'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.lambda-nft-marketplace
        nft-contract
        name))))
...
(define-public (list-nft (token-id uint) (nft-contract <nft-trait>) (ft-contract <ft-trait>) (price uint))
  (let (
    ...
    (print {event: "nft-listed", token-id: token-id, seller: seller, ft-contract: ft-principal, price: price})
    (ok true)))
...
(define-public (update-listing-ft (token-id uint) (new-ft-contract <ft-trait>) (new-price uint))
  (let (
    ...
    (map-set listings token-id (merge listing {ft-contract: new-ft-principal, price: new-price}))

    (print {event: "listing-updated", token-id: token-id, ft-contract: new-ft-principal, price: new-price})
    (ok true)))
...
(define-read-only (get-royalty-info)
  {percent: (var-get royalty-percent), recipient: (var-get royalty-recipient)})

(define-read-only (get-platform-info)
  {fee: (var-get platform-fee), recipient: (var-get platform-recipient)})
